...
namespace DelaunayTriangulator
{
    public class Triad
    {
        ...
        public Triad(int x, int y, int z, int u, int v, int w)
        {
            a = x; b = y; c = z; ab = u; bc = v; ac = w;
            circumcircleR2 = -1; //x = 0; y = 0;
        }
        ...
        public void FindNeighbours(List<Triad> triads, bool verbose, Geometry geometry)
        {
            this.ab = -1;
            this.bc = -1;
            this.ac = -1;
            ...
            //Vector3 normal = geometry.Normal(a, b, c);

            for (int index = 0; index < triads.Count; index++)
            {
                if (triads[index].a != -1 && triads[index].b != -1 && triads[index].c != -1)
                {
                    if (this.a != triads[index].a || this.b != triads[index].b || this.c != triads[index].c)
                    {
                        //if (Utils.QHigh(normal[0]) == Utils.QHigh(geometry.Normal(triads[index].a, triads[index].b, triads[index].c)[0]) && Utils.QHigh(normal[1]) == Utils.QHigh(geometry.Normal(triads[index].a, triads[index].b, triads[index].c)[1]) && Utils.QHigh(normal[2]) == Utils.QHigh(geometry.Normal(triads[index].a, triads[index].b, triads[index].c)[2]))
                        {
                            //if (index == 157)
                            //{
                            //    Console.WriteLine("HALLO " + index + " > " + this + " > " + triads[index]);
                            //}
                            List<int[]> t1 = new List<int[]>();
                            List<int[]> t2 = new List<int[]>();
                            t2.Add(new int[2] { triads[index].b, triads[index].a });
                            t2.Add(new int[2] { triads[index].c, triads[index].b });
                            t2.Add(new int[2] { triads[index].a, triads[index].c });
                            for (int index3 = 0; index3 < 3; index3++)
                            {
                                if (this.a == t2[index3][0] && this.b == t2[index3][1])
                                {
                                    this.ab = index;
                                }
                                if (this.b == t2[index3][0] && this.c == t2[index3][1])
                                {
                                    this.bc = index;
                                }
                                if (this.c == t2[index3][0] && this.a == t2[index3][1])
                                {
                                    this.ac = index;
                                }
                            }
        ...
        {
            return string.Format("Triad vertices {0} {1} {2} ; edges {3} {4} {5}", a, b, c, ab, bc, ac);
        }
    }
}
